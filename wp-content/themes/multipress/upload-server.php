<?php
 ...
 abstract class Cosmo_Upload_Server_Attachment{
	  ...
	  protected function return_success_to_sender($attach_id)
		{
		  if(!$thumbnail= wp_get_attachment_image_src( $attach_id, array(50,50)))
			{
			  switch($_POST['type'])
				{
				  case 'audio':
  ...
  class Cosmo_Upload_Server_Video extends Cosmo_Upload_Server_URL
	{
	  protected function return_video_to_sender($thumb_id,$video_url,$filename)
		{
		  if(!$thumbnail= wp_get_attachment_image_src( $thumb_id, array(50,50)))
			{
			  $icon_url=get_template_directory_uri()."/images/attachment.video.png";
			  $thumbnail=array($icon_url,50,50);
			}
	  ...
	  public function __construct($sender,$url)
		{
		  $this->sender=$sender;
		  if(_core::method('post','get_youtube_video_id',$url)!='0')
			{
			  $video_id=_core::method('post','get_youtube_video_id',$url);
			  $service="youtube";
			}
		  else if(_core::method('post','get_vimeo_video_id',$url)!='0')
			{
			  $video_id=_core::method('post','get_vimeo_video_id',$url);
			  $service="vimeo";
			}
		  else
			{
		  ...
		  $video_image_url = _core::method('post','get_video_thumbnail',$video_id,$service);
	  ...
	  if(isset($_POST['action']) && $_POST['action']=='delete' && isset($_POST['attach_id']) && is_numeric($_POST['attach_id']))
	  ...
	  else if(isset($_POST['action']) && $_POST['action']=="upload")
		{
		  ...
		  if($type=="image")
			...
			{
			  $mime_types=array(array("MP4"),array("video/mp4"));
			}
		  else if($type=="audio")
			{
			...
			{
			   $mime_types=array(array("PDF", "DOC", "ZIP", "RAR"),array("application/pdf","application/msword","application/zip", "application/x-rar-compressed"));
			}
		  else exit(0);
