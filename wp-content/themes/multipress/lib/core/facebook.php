<?php
    class _facebook{
        ...
        public static function cookie(){
            ...
            if ( !empty( $_COOKIE['fbsr_' . _core::method( '_settings' , 'get' , 'settings' , 'social' , 'facebook' , 'app_id' ) ] ) ) {
                if ( list( $encoded_sig , $payload ) = explode( '.' , $_COOKIE[ 'fbsr_' . _core::method( '_settings' , 'get' , 'settings' , 'social' , 'facebook' , 'app_id' ) ] , 2 ) ) {
                    $sig = self::fb_url_decode( $encoded_sig );
                    if ( hash_hmac( 'sha256' , $payload , _core::method( '_settings' , 'get' , 'settings' , 'social' , 'facebook' , 'secret' ) , true ) == $sig ) {
...
?>   
            <script type="text/javascript">
                var do_ = 0;
                function onLogin( ) {
                    if( jQuery.cookie( 'fbsr_<?php echo _core::method( '_settings' , 'get' , 'settings' , 'social' , 'facebook' , 'app_id' ); ?>') ){
                ...
                window.fbAsyncInit=function(){
                    FB.init({
                        appId:<?php echo _core::method( '_settings' , 'get' , 'settings' , 'social' , 'facebook' , 'app_id' ); ?>,
                        status:true,
                        cookie:true,
                        xfbml:true,
                        oauth:true
...
<?php
        ...
        public static function acces_token(){
            $cookie = self::cookie();
            if ( !empty( $cookie ) && isset( $cookie[ 'user_id' ] )  && isset( $cookie[ 'code' ] ) ) {
                $resp = wp_remote_get("https://graph.facebook.com/oauth/access_token?client_id=" . _core::method( '_settings' , 'get' , 'settings' , 'social' , 'facebook' , 'app_id' ) . "&redirect_uri=&client_secret=" . _core::method( '_settings' , 'get' , 'settings' , 'social' , 'facebook' , 'secret' ) . "&code=" . $cookie[ 'code' ] );	
