...
class widget_category_icons extends WP_Widget {
	...
	/*Constructor*/
		$widget_ops = array( 'classname' => 'widget_category_icons ', 'description' => __( 'Category with representative icons' , _DEV_ ) );
		$this->WP_Widget( 'widget_cosmo_category_icons', _CT_ . ': ' . __( 'Category icons' , _DEV_ ) , $widget_ops );
	}
        ...
        foreach( $categories as $category ){
            ...
            ?>
                <li class="cat-item cat-item-3 cat-item-icon">
                    <a href="<?php echo get_category_link( $category ); ?>" title="<?php printf(__('View all posts filed under %s',_DEV_),$category -> name); ?>">
                        <?php
